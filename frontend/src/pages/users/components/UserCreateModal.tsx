  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        message: t('users.messages.create_success'),
      });
      onClose();
      setFormData({
        name: '',
    ...
    >
      <div className="bg-card border border-border w-full max-w-lg rounded-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="text-lg font-bold text-foreground">{t('users.add_user')}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-border">
            ...
            >
              {createMutation.isPending ? t('users.table.loading') : t('users.table.save')}
            </Button>
          </div>
        </form>
      </div>
